...
export default function ProdutoDetalhes() {
  ...
  const calcularFrete = async () => {
    ...
    try {
      ...
      if (uf === 'PE') { valor = 15.90; prazo = 2; }
      else if (['AL', 'PB', 'RN', 'CE', 'BA', 'SE', 'MA', 'PI'].includes(uf)) { valor = 29.90; prazo = 5; }
      else { valor = 54.90; prazo = 10; }
      ...
      <div className="container mx-auto px-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:p-10 flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <button onClick={() => addToCart(produto)} className="w-full bg-[#E21E26] hover:bg-[#c41920] text-white py-5 rounded-xl font-bold text-xl transition-all shadow-lg flex items-center justify-center gap-3 uppercase mb-8">
              Comprar Agora
            </button>
            ...
            <div className="mt-4 border-t border-gray-100 pt-6">
              ...
              <div className="flex mt-3 gap-2">
                <input type="text" maxLength={8} placeholder="00000000" value={cep} onChange={(e) => setCep(e.target.value.replace(/\D/g, ""))}
                ...
                <button onClick={calcularFrete} disabled={loadingFrete} className="bg-gray-800 text-white px-6 py-2 rounded-lg text-xs font-bold uppercase hover:bg-black transition-all">
        ...
        <div className="mt-10 bg-white rounded-xl shadow-sm border border-gray-100 p-8 mb-10">
          <h2 className="text-xl font-bold text-gray-800 mb-6 uppercase border-l-4 border-[#E21E26] pl-4">Especificações Técnicas</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 text-black">
            <div className="flex justify-between border-b border-gray-50 py-2 text-sm">
              ...
              <span className="font-bold">{produto.marca || 'Nexus Gaming'}</span>
            </div>
            <div className="flex justify-between border-b border-gray-50 py-2 text-sm">
